...
Color 0A
cls
title Portable NTLite Launcher - Helper Edition
...
:2
:LaunchNTLite
if not exist ".\bin\ntlite\!arch!\NTLite.exe" set "nag=PLEASE INSTALL NTLITE FIRST" && (goto) 2>nul
title DO NOT CLOSE
cls
...
echo @echo off > quicklaunch_ntlite.bat
echo Color 0A >> quicklaunch_ntlite.bat
echo cls >> quicklaunch_ntlite.bat
echo set "folder=%%CD%%" >> quicklaunch_ntlite.bat
echo if "%%CD%%"=="%%~d0\" set "folder=%%CD:~0,2%%" >> quicklaunch_ntlite.bat
echo set "UserProfile=%%folder%%\data\" >> quicklaunch_ntlite.bat
echo set "AppData=%%folder%%\data\AppData\Roaming\" >> quicklaunch_ntlite.bat
echo set "LocalAppData=%%folder%%\data\AppData\Local\" >> quicklaunch_ntlite.bat
echo set "arch=x86" >> quicklaunch_ntlite.bat
echo if exist "%%PROGRAMFILES(X86)%%" set "arch=x64" >> quicklaunch_ntlite.bat
echo cls >> quicklaunch_ntlite.bat
echo echo YOU REQUIRE ADMIN BTW PRESS ENTER TO CONTINUE ^& pause^>nul >> quicklaunch_ntlite.bat
echo start .\bin\ntlite\%%arch%%\NTLite.exe >> quicklaunch_ntlite.bat
echo exit >> quicklaunch_ntlite.bat
echo A QUICKLAUNCHER HAS BEEN WRITTEN TO: quicklaunch_ntlite.bat
...
if exist NTLite_setup_x64.exe del NTLite_setup_x64.exe
if exist NTLite_setup_x86.exe del NTLite_setup_x86.exe
...
:Credits
cls
if exist .\doc\ntlite_license.txt (goto) 2>nul
echo ================================================== > .\doc\ntlite_license.txt
echo =              Script by MarioMasta64            = >> .\doc\ntlite_license.txt
echo =           Script Version: v%current_version%- release        = >> .\doc\ntlite_license.txt
echo ================================================== >> .\doc\ntlite_license.txt
echo =You may Modify this WITH consent of the original= >> .\doc\ntlite_license.txt
echo = creator, as long as you include a copy of this = >> .\doc\ntlite_license.txt
echo =      as you include a copy of the License      = >> .\doc\ntlite_license.txt
echo ================================================== >> .\doc\ntlite_license.txt
echo =    You may also modify this script without     = >> .\doc\ntlite_license.txt
echo =         consent for PERSONAL USE ONLY          = >> .\doc\ntlite_license.txt
echo ================================================== >> .\doc\ntlite_license.txt
cls
title Portable NTLite Launcher - Helper Edition - About
for /f "DELIMS=" %%i in (.\doc\ntlite_license.txt) do (echo %%i)
pause
call :PingInstall
(goto) 2>nul
...
call :HelperDownload "https://raw.githubusercontent.com/MarioMasta64/EverythingPortable/master/launch_ntlite.bat" "launch_ntlite.bat.1"
cls & if exist launch_ntlite.bat.1 goto ReplacerCreate
...
echo del launch_ntlite.bat >> replacer.bat
echo rename launch_ntlite.bat.1 launch_ntlite.bat >> replacer.bat
echo start launch_ntlite.bat >> replacer.bat
