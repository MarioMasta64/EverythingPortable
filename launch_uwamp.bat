...
:2
:LaunchUwAmp
if not exist ".\bin\UwAmp\UwAmp.exe" set "nag=PLEASE INSTALL UWAMP FIRST" && (goto) 2>nul
title DO NOT CLOSE
cls
...
for /d %%i in (".\bin\UwAmp\*") do if /i not "%%i"==".\bin\UwAmp\bin" rmdir /s /q "%%i"
(goto) 2>nul
...
taskkill /f /im mysqld.exe
taskkill /f /im UwAmp.exe
rmdir /s /q \bin\UwAmp\bin\
echo y | del .\bin\UwAmp\*.*
del .\extra\UwAmp.zip
(goto) 2>nul
...
echo @echo off > quicklaunch_uwamp.bat
echo Color 0A >> quicklaunch_uwamp.bat
echo cls >> quicklaunch_uwamp.bat
echo set "folder=%%CD%%" >> quicklaunch_uwamp.bat
echo if "%%CD%%"=="%%~d0\" set "folder=%%CD:~0,2%%" >> quicklaunch_uwamp.bat
echo set "UserProfile=%%folder%%\data\" >> quicklaunch_uwamp.bat
echo set "AppData=%%folder%%\data\AppData\Roaming\" >> quicklaunch_uwamp.bat
echo set "LocalAppData=%%folder%%\data\AppData\Local\" >> quicklaunch_uwamp.bat
echo cls >> quicklaunch_uwamp.bat
...
if exist UwAmp.zip del UwAmp.zip
...
:Credits
cls
if exist .\doc\uwamp_license.txt (goto) 2>nul
...
call :HelperDownload "https://raw.githubusercontent.com/MarioMasta64/EverythingPortable/master/launch_uwamp.bat" "launch_uwamp.bat.1"
cls & if exist launch_uwamp.bat.1 goto ReplacerCreate
